...
class LatentMoETransition(nn.Module):
    """Latent transition for COCONUT latent reasoning evaluation."""
    ...
    def __init__(
        ...
        excluded_expert_indices: Optional[Sequence[int]] = None,
    ):
        super().__init__()
        hidden_size = int(hidden_size)
        num_experts = max(int(num_experts), 1)
        top_k = min(max(int(top_k), 1), num_experts)
        ...
        excl_mask = torch.zeros(num_experts, dtype=torch.bool)
        if excluded_expert_indices:
            for idx in excluded_expert_indices:
                i = int(idx)
                if 0 <= i < num_experts:
                    excl_mask[i] = True
        active = num_experts - int(excl_mask.sum().item())
        if active < top_k:
            raise ValueError(
                f"LatentMoETransition: after excluding experts {tuple(excl_mask.nonzero(as_tuple=True)[0].tolist())}, "
                f"only {active} experts remain but top_k={top_k}."
            )
        self.register_buffer("_router_exclude_mask", excl_mask, persistent=False)
        ...
        self.shared_expert = ExpertMLP(hidden_size) if self.use_shared_expert else None
    ...
    def forward(
        ...
        if bool(self._router_exclude_mask.any()):
            mask = self._router_exclude_mask.to(device=gate_logits.device, dtype=torch.bool)
            gate_logits = gate_logits.masked_fill(mask.unsqueeze(0), float("-inf"))
